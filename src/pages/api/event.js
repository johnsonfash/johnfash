...
import fs from 'fs'
const { host, user, password, database, timezone, ACCESS_TOKEN } = process.env;
...
const addEvent = async (req) => {
    try {
        ...
        const [rows] = await mysql.execute('INSERT INTO `events` (`image`, `title`, `bible_verse`, `time`, `location`) VALUES (?,?,?,?,?)', [fileData?.data?.image, fileData?.data?.title, fileData?.data?.bible_verse, fileData?.data?.time, fileData?.data?.location]);
        mysql.end();
        if (rows) {
            return { error: false, errorMessage: null, data: null };
        } else {
            return { error: true, errorMessage: "unable to add event, please try again!", data: null }
        }
    } catch (err) {
...
const getEvents = async () => {
    try {
        const mysql = await createConnection({ host, password, user, database });
        const [rows] = await mysql.execute('SELECT * FROM `events`');
        mysql.end();
...
const deleteEvent = async (id) => {
    try {
        const mysql = await createConnection({ host, password, user, database });
        await mysql.execute('DELETE FROM `events` WHERE `id` = ?', [id * 1]);
        mysql.end();
        return { error: false, errorMessage: null, data: null };
    } catch (err) {
        return { error: true, errorMessage: err.message }
...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        const response = await addEvent(req)
        res.send(response)
    }
    if (req.method === 'GET') {
